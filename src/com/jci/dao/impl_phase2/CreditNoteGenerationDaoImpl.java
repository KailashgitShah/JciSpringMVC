...
//                         
                             ...
                             String sql = "SELECT  count(*) FROM jcicredit_note  where gstCode='"+gstGstCode+"'\r\n"
                                                          ...
                                                          + "AND YEAR(Creation_date) = YEAR(GETDATE()) - CASE WHEN MONTH(GETDATE()) < 4 THEN 1 ELSE 0 END AND Crn_status = 0;";
...
//                                                      + "WHERE ISNUMERIC(SUBSTRING(Credit_note_no, 4, 6)) = 1 and Crn_status = 0;";
                             ...
                             String sql = "SELECT \r\n"
                                                          ...
                                                          + "    END \r\n"
                                                          + "FROM jcicredit_note\r\n"
                                                          + "WHERE ISNUMERIC(SUBSTRING(Credit_note_no, 4, 6)) = 1\r\n"
                                                          + "AND Creation_date >= DATEFROMPARTS(YEAR(GETDATE()) - CASE WHEN MONTH(GETDATE()) < 4 THEN 1 ELSE 0 END, 4, 1)  -- Start of current financial year (April 1st)\r\n"
                                                          + "AND Creation_date < DATEFROMPARTS(YEAR(GETDATE()) + CASE WHEN MONTH(GETDATE()) >= 4 THEN 1 ELSE 0 END, 4, 1)  -- Before next financial year (April 1st next year)\r\n"
                                                          + "AND YEAR(Creation_date) = YEAR(GETDATE()) - CASE WHEN MONTH(GETDATE()) < 4 THEN 1 ELSE 0 END and Crn_status = 0;  -- Ensure it's the current financial year\r\n"
                                                          + "";
                             return (int) currentSession().createSQLQuery(sql).uniqueResult();
              }
