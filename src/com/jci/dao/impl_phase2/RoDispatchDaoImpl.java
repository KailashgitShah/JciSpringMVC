...
public class RoDispatchDaoImpl implements RoDispatchDao {
	...
	public List<String> loadAllDiNo() {
		...
		String sqlString = " SELECT a.DI_no, " + " a.TotalSum, " + " a.Regional_office, " + " b.roSum" + "	FROM ("
				+ "	    SELECT DI_no,"
				+ "    SUM(Gr1_qty + Gr2_qty + Gr3_qty + Gr4_qty + Gr5_qty + Gr6_qty + Gr7_qty + Gr8_qty) AS TotalSum, "
				+ "  Regional_office " + " FROM jciDI_ho " + " GROUP BY DI_no, Regional_office " + " 	) a "
				+ "	 LEFT JOIN ( " + "	    SELECT HO_DI_NO, "
				+ "	           SUM(Gr1_qty + Gr2_qty + Gr3_qty + Gr4_qty + Gr5_qty + Gr6_qty + Gr7_qty + Gr8_qty) AS roSum "
				+ "	    FROM jciDI_ro " + "    GROUP BY HO_DI_NO " + "	) b " + "	ON a.DI_no = b.HO_DI_NO "
				+ " WHERE(a.TotalSum > b.roSum or roSum is NULL) " + " AND a.Regional_office = '" + regionCode + "';";
...
//To get Cooperatives
	...
	public List<String> getCooperative(String regionIdString) {
		String sqlString = "SELECT CENTER_CODE, centername " + " FROM jcipurchasecenter " + "WHERE rocode = '"
				+ regionIdString + "'" + " ";
		List<Object[]> list = currentSession().createSQLQuery(sqlString).list();
		List<String> newlList = new ArrayList<>();
		for (Object[] ro : list) {
...
//TO get previous DI  for RO
	...
	public String dpcCheck(String dpc, String hoDIno) {
		// TODO Auto-generated method stub
		String sqlString = "SELECT CASE WHEN EXISTS " + " (SELECT * FROM jciDI_ro WHERE HO_DI_NO = '" + hoDIno
				+ "' AND DPC = '" + dpc + "') " + " THEN '0' ELSE '1' END AS result;";
