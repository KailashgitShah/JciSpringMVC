...
public class generationOfCashAgainstDispatchDocumentDaoImpl implements generationOfCashAgainstDispatchDocumentDao {
	...
	public List<TopSheetDto> getTopSheetDatacashAgainstDispatchDocument(String topSheetGeneratedId) {
		...
		String sqlQuery = "SELECT DISTINCT " + "    jcitopsheet.billOfSupplyNo, " + "    jcitopsheet.invoiceValue, "
				+ "    jcitopsheet.contract_identification_no, " + "    jcitopsheet.millname, "
				+ "    jcitopsheet.millcode, " + "    jcitopsheet.topSheetCreateDate, " + "    jcitopsheet.hodiNo, "
				+ "    jcitopsheet.hodiDate, " + "    jcitopsheet.contract_no, " + "    CONCAT( "
				...
				+ "    ) AS Formatted_Date_of_shipment, " + "    jb.Challan_No " + "FROM " + "    jcitopsheet "
				+ "INNER JOIN " + "    jcidispatch_details d ON d.Contract_No = jcitopsheet.contract_no "
				+ "INNER JOIN " + "    jcibos_generation jb ON jb.Bill_of_supply_no = jcitopsheet.billOfSupplyNo "
				+ "WHERE " + "    jcitopsheet.topsheet_generated_id = '" + topSheetid + "'"; // Using parameterized
																								// query
...
//				        
	...
	public List<boenonlcDTO> getBOENONLC(String contractno) {
		...
		String sqlQuery = "SELECT " + "    billOfSupplyNo, " + "    bosDate, " + "    invoiceValue, "
				+ "    contract_no, " + "    millcode, " + "    millname, " + "    milladdress, "
				+ "    contract_date, " + "    cropYear, " + "    topSheetCreateDate " + "FROM " + "    jcitopsheet "
				+ "WHERE " + "    topsheet_generated_id = '" + contract + "'";
	...
	public List<Object[]> listdetailsbillofsuppllycash(String st) {
		String sql = "SELECT DISTINCT " + "    g.Bill_of_supply_no, " + "    g.BOS_date, " + "    g.Invoice_value, "
				+ "    g.Challan_No, " + "    g.millcode, " + "    m.unit_name, " + "    m.unit_address1, "
				+ "    c.Contract_identification_no, " + "    c.Contract_no, " + "    c.Contract_date, "
				+ "    c.CropYear, " + "    di.DI_no, " + "    di.DI_Date " + "FROM " + "    jcibos_generation g "
				+ "    INNER JOIN jcicontract c ON c.Contract_no = '" + st + "'"
				+ "    INNER JOIN jcimilldetailchild m ON m.client_unit_code = g.millcode "
				+ "    INNER JOIN jciDI_ho di ON di.Contract_No = '" + st + "' " + "WHERE " + "    g.Contract_no = '"
				+ st + "' AND NOT EXISTS (" + "        SELECT 1 " + "        FROM jcitopsheet t "
	...
	public List<Object[]> listdetailsofpaymemt1(String st) {
		String sql = "SELECT " + "    bank, " + "    branch, " + "    ifsc, "
				+ "    CONVERT(varchar, Instrument_Date, 103) AS Instrument_Date, " + "    Instrument_No, "
				+ "    Instrument_value, " + "    Supporting_document, " + "    Auto_revolving_amount, "
				+ "    CONVERT(varchar, Expiry_date, 103) AS Expiry_date, "
				+ "    CONVERT(varchar, Last_shipment_date, 103) AS Last_shipment_date, " + "    Payment_type, "
				+ "    CONVERT(varchar, PaymentDue_date, 103) AS PaymentDue_date, "
				+ "    jcicontract.Contract_acceptance_doc " + "FROM " + "    jcipayment_arrangement "
				+ "    INNER JOIN jcicontract ON jcicontract.Contract_no = '" + st + "' " + "WHERE "
	...
	public List<Object> contractonmill1(String millCode) {
		String sql = "SELECT DISTINCT Contract_No FROM jcipayment_arrangement where\r\n" + "   millcode='"
				+ millCode + "' and Payment_type <> 'Letter_of_Credit'";
...
//		}
	...
	public String getNominalWt(String bos) {
		...
		String sql = "SELECT SUM(nominal_qty) AS total_nominal_qty " + "FROM jcidispatch_details_child "
	...
	public List<TopsheetDetailsModel> getAlldetails(String topSheetIdGenerated) {
		...
		String sqlQuery = "  select billOfSupplyNo , invoiceValue ,topsheet_generated_id from jcitopsheet WHERE topsheet_generated_id = '"
				+ topSheetIdGenerated + "'";
	...
	public Date getInstrumentDate(String ContractNo) {
		String sql = "SELECT  Instrument_Date from jcipayment_arrangement where  Contract_No = '" + ContractNo
				+ "' AND Payment_type <> 'Letter_of_Credit'";
		...
		// String resultList1=
		// (String)this.sessionFactory.getCurrentSession().createSQLQuery(sql).list();
	...
	public List<Object[]> fetchMill_Name() {
		String sql = "SELECT DISTINCT s.client_name, s.client_unit_code\r\n"
				...
				+ ") AS s\r\n"
				+ "INNER join  jcibos_generation as b on b.millcode=s.client_unit_code inner join \r\n"
				+ " jcipayment_arrangement c on b.millcode = c.millcode and c.Payment_type <> 'Letter_of_Credit'";
