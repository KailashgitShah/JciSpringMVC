...
<body class="fixed-navbar">
							...
							<div class="ibox-body">
								<form id="myForm" action="savenominal.obj" method="POST">
									...
									<div class="row">
										...
										<div class="col-sm-4 form-group" id="contractNoContainer"
											style="display: none;">
									...
									<div class="row">
										...
										<div class="col-sm-4 form-group">
											...
											<label>O&M Official</label> <span class="text-danger">*</span>
									...
									<div class="row">
										<div class="col-sm-4 form-group">
											<input type="hidden" class="form-control taxtbox"
												value="${total}" name="Settlement_id_generated" type="text"
					...
					$("#submit").click(
							function() {
								var omMessageElement = document
										.getElementById("omMessage").innerText;
								if (omMessageElement.length > 0) {
									alert("Please Choose Different OM");
									return false;
								}
								//return false;
							})
		...
		$("#DateofInpection").datepicker(
				{
					...
					onSelect : function(selectedDate) {
						...
						var formattedDate = ("0" + currentDate.getDate())
								.slice(-2)
								+ "-"
						...
						var selectedDateObject = new Date(selectedDate.split(
								"-").reverse().join("-")); // Convert selected date to Date object
						...
						checkAvailablitiyOFOm(selectedOption);
	...
	<script>
		function checkAvailablitiyOFOm(dateOfInspection) {
			$
					.ajax({
						type : 'GET',
						...
						success : function(data) {
							...
							if (SelectedMillName == ''
									|| SelectedMillName == null) {
								alert("Please select mill first");
							...
							for (var i = 0; i < response.length; i++) {
								...
								var millCode = innerArray[2].split('---')[1];
								var millName = innerArray[2].split('---')[0];
							...
							for (var i = 0; i < response.length; i++) {
								...
								if (omoofficial == selectedOmofficial
									...
									omMessageElement.innerText = selectedOmofficial
											+ " is Already Occupied On This Date For Another Claim Settlement For Mill Name : "
											+ millNamesString;
														...
														.ajax({
															...
															success : function(
																...
																for (var i = 0; i < dataArray.length; i++) {
																	var date = new Date(
																			dataArray[i][2]); // Parsing the date string into a Date object
																	var formattedDate = ("0" + date
																			.getDate())
																			.slice(-2)
																			+ '-'
																			+ ("0" + (date
																			...
																			+ '-'
																			+ date
																					.getFullYear(); // Formatting to dd-mm-yyyy
																	var dateshipment = new Date(
																			dataArray[i][4]);
																	var formatShipmentDate = ("0" + dateshipment
																			.getDate())
																			.slice(-2)
																			+ '-'
																			+ ("0" + (dateshipment
																			...
																			+ dateshipment
																					.getFullYear();
																	var newRow = "<tr>";
																	newRow += '<td><input type="checkbox" onclick="myFunction(this)" id="checking'
																			+ i
																			+ '" class="row-checkbox" name="rowCheckbox'
																			+ i
																			+ '" value="0"></td>'
																			+ '<td><div class="table-cell"><input type="hidden"  name="challans[]" value="' + dataArray[i][0] + '">'
																			+ dataArray[i][0]
																			+ '</div></td>'
																			+ '<td><div class="table-cell"><input type="hidden"  name="mr_no[]" value="' + dataArray[i][1] + '">'
																			+ dataArray[i][1]
																			+ '</div></td>'
																			+ '<td><div class="table-cell"><input type="hidden"  name="mr_date[]" value="' + formattedDate + '">'
																			+ formattedDate
																			+ '</div></td>'
																			+ '<td><div class="table-cell"><input type="hidden"  name="billofsupply[]" value="' + dataArray[i][3] + '">'
																			+ dataArray[i][3]
																			+ '</div></td>'
																			+ '<td><div class="table-cell"><input type="hidden"  name="dateofshipment[]" value="' + formatShipmentDate + '">'
																			+ formatShipmentDate
																			+ '</div></td>'
																			+ '<td><div class="table-cell"><input type="hidden"  name="shipmentquantity[]" value="' + dataArray[i][5] + '">'
																			+ dataArray[i][5]
																			+ '</div></td>'
																			+
