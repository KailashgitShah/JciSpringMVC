<%@page import="com.jci.model.CreditNoteSettled"%>
...
<%
List<CreditNoteSettled> list = (List<CreditNoteSettled>) request.getAttribute("list");
%>
						...
						<div class="scrollmenu">
							<table class="table table-striped table-bordered table-hover"
								...
								<tbody>
									<%
									int i = 1;
									for (CreditNoteSettled crn : list) {
										String path = crn.getDoc();
										System.err.println("path =>" + path);
									%>
									<tr>
										<td><%=i%></td>
										<td><%=crn.getCreditNoteDate()%></td>
										<td><%=crn.getCreditNoteNo()%></td>
										<td><%=crn.getMrDate()%></td>										
										<td><%=crn.getMrNo()%></td>
										<td><%=crn.getVarityGrade()%></td>
										<td><%=crn.getSettlementAmt()%></td>
										<td><a class="btn-success" target="_blank"
											href='downloadCreditNoteSettledDocs.obj?imagePath=<%=path%>'>View</a></td>
										<td><a
											onclick="return confirm('Are you sure you want to cancle this Credit Note?');"
											href="changeCrnStatus.obj?crnId=<%=crn.getId()%>"
